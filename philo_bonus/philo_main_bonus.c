...
/*   Updated: 2022/10/22 22:57:07 by jyao             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	kill_sigterm_all(t_philo_global *philo_g_ptr)
{
	int	i;
	...
	while (i < philo_g_ptr->args_main[NOM_PHILO])
	{
		kill(philo_g_ptr->all_pid[i], SIGKILL);
		i++;
	}
}
...
int	main(int argc, char *argv[])
{
	...
	sem_wait(philo_global.sem_exit);
	kill_sigterm_all(&philo_global);
